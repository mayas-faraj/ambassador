...
import textImage from '../assets/imgs/text.png';
	...
	return (
		<Fragment key={slug}>
			<header className="book-header">
				<Link to="/adad/#library" className="book-header__menu">
					...
					<div className="book">
						<div className="book-review">
									...
									<div className="book-list" ref={listRef}>
										<div className="book-list-wrapper">
										{
											books.filter(book=>book.image!=null && book.image!="").reverse().map(book=>book.image && (
												<Link key={book.slug} className="book-item" to={"/adad/libri/"+book.slug} title={book.title} ref={book.slug===slug?itemRef:null}>
													<img src={book.image?context.uploadsUrl+"/"+book.image:transparentImage} className="book-item__image" alt={book.title} />
												</Link>
											))
										}
										</div>
							...
							<div className="book-info">
								<img className="book-info__img" src={book.image?context.uploadsUrl+"/"+book.image:transparentImage} alt={book.title} />
								<div className="book-info-contentiner">
									<h1 className="book-info__title">{book.title}</h1>
									{book.author && <div className="book-info__meta">editore: {book.author}</div>}
									{book.city && <div className="book-info__meta">luogo: {book.city}</div>}
									{book.year && <div className="book-info__meta">anno: {(book.month!=null && book.month>=0) && [ "Gennaio", "febbraio", "Marzo", "aprile", "Maggio", "Giugno", "Luglio", "agosto", "settembre", "ottobre", "novembre", "Dicembre" ][book.month]}, {book.year}</div>}

																		{book.language && <div className="book-info__meta">lingua: {book.language}</div>}
									{book.series && <div className="book-info__meta">collana: {book.series}</div>}
									{book.pages_count && book.pages_count!=="0" && <div className="book-info__meta">numero di pagine: {book.pages_count}</div>}
									{book.comment && <div className="book-info__meta">{book.comment.replaceAll("\\n", "\n")}</div>}
									{book.format && <div className="book-info__meta">formato: {book.format}</div>}
									{book.isbn && <div className="book-info__meta">ISBN: {book.isbn}</div>}
							...
							<div className="book-content">
								<p className="book-content__text">{book.content!="" && book.content || "il contenuto sarà disponibile il prima possibile"}</p>
							</div>
						</div>
						{
							book.book_file && (
							<div className="book-browse-basic">
								...
								<p className="book-browse__text">contenuti</p>
								<Dialog open={bookModal} onClose={()=>setBookModal(false)} maxWidth={"lg"} fullWidth={true}>
									<DialogContent>
										<a href={context.booksUrl+"/"+book.book_file} className="book-browse__fullscreen-link" target="_blank">
											<FullScreenIcon/>
										</a>
										<iframe className="book-browse__iframe" src={context.booksUrl+"/"+book.book_file}/>
									</DialogContent>
								</Dialog>
							</div>
						)}
						{
							book.revision_file && (
							<div className="book-browse-revision">
								...
								<p className="book-browse__text">recensioni</p>
								<Dialog open={revisionModal} onClose={()=>setRevisionModal(false)} maxWidth={"lg"} fullWidth={true}>
									<DialogContent>
										<a href={context.booksUrl+"/"+book.revision_file} className="book-browse__fullscreen-link" target="_blank">
											<FullScreenIcon/>
										</a>
										<iframe className="book-browse__iframe" src={context.booksUrl+"/"+book.revision_file}/>
									</DialogContent>
								</Dialog>
