...
const SOCIALS=[
	{
		"app": "linkedin", 
		"enabled": true,
		"icon": <LinkedinButtonIcon className="social-linkedin"/>
	},
	{
		"app": "twitter", 
		"enabled": true,
		"icon": <TwitterButtonIcon className="social-twitter"/>
	},
	{
		"app": "facebook", 
		"enabled": true,
		"icon": <FacebookButtonIcon className="social-facebook"/>
	},
	{
		"app": "youtube", 
		"enabled": true,
		"icon": <YoutubeButtonIcon className="social-youtube"/>
	},
	{
		"app": "instagram", 
		"enabled": false,
		"icon": <InstagramButtonIcon className="social-instagram"/>
	}
];
			...
			<div className="blog">
				...
				<div className="blog-content">
					<div className="blog-socials">
						<h3 className="blog-social__text">Partecipa anche tu<br/>con l'Ambasciatore<br/>sui social media</h3>
						<div className="blog-social-wrapper">
							<MobileImage className="blog-social-mobile-cover" />
							{
						...
						{
							SOCIALS.map(social=>(
								<div key={social.app} className="blog-social-icon">
					...
					<div className="blog-view">
						...
						{ this.state.blogs!=null && this.state.blogs.length===0 && <p className="blog-noitems">gli articoli arriveranno presto</p> }
						{ this.state.blogs!=null && this.state.blogs.status!=="failed" && this.state.blogs.map((blog, index)=>(
							(index>=this.state.blogIndex && index<this.state.blogIndex+VISIBLE_BLOGS_COUNT) && <div key={blog.title+blog.image} className="blog-item">
								<a className="blog-item__link" href={blog.source_link?blog.source_link:""} target="_blank" rel="noreferrer">
									<img src={this.context.uploadsUrl+"/"+blog.image} className="blog-item__image" alt={blog.title}/>
									<div className="blog-info">
										<span className="blog-info__category">{blog.type}</span>
										<h3 className="blog-info__title">{blog.title.replaceAll("\\n", "\n")}</h3>
										<svg className="blog-info__seperator">
											<line x1="0" y1="0" x2="100%" y2="0"/>
											<line x1="0" y1="100%" x2="100%" y2="100%"/>
										</svg>
										
										<div className="blog-info__source">{(blog.source_name?blog.source_name + " / ":"") + (blog.post_date!=="0000-00-00 00:00:00"?this.formatDate(new Date(blog.post_date)):"")}</div>
									</div>
								</a>
							</div>
						)) }
