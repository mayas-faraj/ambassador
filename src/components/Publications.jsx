...
class Publications extends React.Component {
	...
	render() {
		return (
			<div className="publications" ref={this.props.setRef}>
				<div className="publications-header">
					<div className="publications-top">
						<div key={this.state.activeBookIndex} className="publications-top__desc">
						...
						{this.state.activeBookIndex>=0 && this.state.books.length>0 && this.state.activeBookIndex<this.state.books.length && (
							<Fragment>
								<h4 className="publications-top__book-title">{this.state.books[this.state.activeBookIndex].title.toUpperCase()}</h4>
								<p className="publications-top__book-preface">{this.state.books[this.state.activeBookIndex].preface}</p>
							</Fragment>)}
						</div>
						{
							this.state.activeCategoryIndex===0 && (
								<p className="publications-top__mark">
								Libri, Memorie, Opere narrative,<br/>
								Libri fotografici
								<div className="publications-top__mark-white">
					...
					<h2 className="publications-header__title">{this.state.categories!=null && this.state.categories[this.state.activeCategoryIndex]!=null && this.state.categories[this.state.activeCategoryIndex].name}</h2>
				</div>
				<div className="publications-content">
					...
					{ this.state.books!==null && (
						...
						{ this.state.books.length===0 && <p className="publications-noitems">gli articoli arriveranno presto</p> }
						{ this.state.books.status!=="failed" && this.state.books.status!=="error" && this.state.books.map((book, index)=>(
								<SwiperSlide key={book.title}>
										<Link className="publications-item" onMouseEnter={()=>this.bookHoverEventHandler(index)} to={"/adad/"+this.state.categories[this.state.activeCategoryIndex].name+"/"+book.slug}>
											<p className={"publications-item__content"+(/[ا-ي]/.test(book.title)?" publications-item__content--arabic":"")}>
											{book.title}
											</p>
											<img src={book.image?this.context.uploadsUrl+"/"+book.image:transparentImage} className="publications-item__image" alt={book.title} />
											<h3 className="publications-item__title">{(book.month?(parseInt(book.month)+1)+"/":"")+book.year}</h3>
										</Link>
								</SwiperSlide>
						)) }
						</Swiper>
